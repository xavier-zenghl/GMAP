...
def train(config_path: str):
    ...
    mc = cfg["model"]
    model = SegNet(n_points=mc["n_points"], scales=[(s["n_centers"], s["k_neighbors"]) for s in mc["scales"]], embed_dim=mc["embed_dim"], depth=mc["depth"], heads=mc["heads"], n_parts=mc["n_parts"]).to(device)
    ...
    for epoch in range(cfg["training"]["epochs"]):
        ...
        for batch in train_loader:
            ...
            optimizer.step()
        if (epoch + 1) % 10 == 0:
            save_checkpoint({"epoch": epoch, "model": model.state_dict()}, f"checkpoints/segnet/epoch_{epoch+1}.pth")
