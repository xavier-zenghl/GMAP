...
def train(config_path: str):
    ...
    mc = cfg["model"]
    model = ParaNet(n_points=mc["n_points"], scales=[(s["n_centers"], s["k_neighbors"]) for s in mc["scales"]], embed_dim=mc["embed_dim"], depth=mc["depth"], heads=mc["heads"], n_parts=mc["n_parts"]).to(device)
    ...
    writer = SummaryWriter("runs/paranet")
    for epoch in range(cfg["training"]["epochs"]):
        model.train()
        for batch in train_loader:
            points = batch["points"].to(device)
            seg_pred = batch["seg_label"].to(device)
            targets = {
                "joint_type": batch["joint_type"].to(device),
                "axis_direction": batch["axis_direction"].to(device),
                "axis_position": batch["axis_position"].to(device),
                "joint_state": batch["joint_state"].to(device),
            }
            loss_dict = model.compute_loss(points, seg_pred, targets)
            optimizer.zero_grad()
            loss_dict["loss"].backward()
            optimizer.step()
        if (epoch + 1) % 10 == 0:
            save_checkpoint({"epoch": epoch, "model": model.state_dict()}, f"checkpoints/paranet/epoch_{epoch+1}.pth")
            logger.info(f"Epoch {epoch+1}: loss={loss_dict['loss'].item():.4f}")
    writer.close()

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", default="configs/paranet.yaml")
